...
create table albums (id SERIAL primary key , title varchar(50) not null, year_of_release DATE not null check(extract(year from year_of_release) > 1900));
...
create table Collections (id SERIAL primary key , title varchar(50) not null, year_of_release DATE not null check(extract(year from year_of_release) > 1900));
